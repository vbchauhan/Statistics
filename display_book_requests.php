...
$this_page = ($start/$display)+1;
	...
	<tr><td align="right" colspan ="16" style="font-weight:bold">Page '.$this_page.' of '.$pages.'</td></tr>';
	echo '<tr bgcolor="#ffc240">
...
echo '<tr><td align = "center" colspan="16">';
if($pages >1)
{
...
if($current_page != 1)
{
echo '<a href="display_book_requests.php?sort='.$sort.'&s='.($start-$display).'&p='.$pages.'">Previous</a>';
}
for($i=1; $i<=$pages; $i++)
{
if($i != $current_page)
{
echo ' <a href="display_book_requests.php?sort='.$sort.'&s='.(($display * ($i - 1))).'&p='.$pages.'">'.$i.'</a> ';
}
else
{
echo $i;
...
{
echo '<a href="display_book_requests.php?sort='.$sort.'&s='.($start+$display).'&p='.$pages.'"> '.Next.' </a>';
}
}
